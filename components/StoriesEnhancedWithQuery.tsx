...
const StoryItem = ({
  ...
  // Safety check - if no stories available, don't render
  if (!userStories?.stories?.length || !latestStory) {
    return null;
  }
...
const StoriesEnhancedWithQuery = () => {
  ...
  // Prefetch stories on mount (guard against network errors)
  useEffect(() => {
    if (userId) {
      // Don't call prefetch in useEffect as it uses hooks internally
      // The useStoriesFeed hook above will handle the initial fetch
      //// console.log("User authenticated, stories will be fetched by useStoriesFeed hook");
    }
  }, [userId]);
  ...
  const handlePostStory = async () => {
    ...
    try {
      ...
      // console.error("Error creating story:", error);
      setLoadingMessage("Error: " + (__error.message || "Failed to create story"));
    ...
    <View style={styles.container}>
        ...
        <View style={styles.previewContainer}>
          ...
          <View style={styles.previewActions}>
            ...
            <TouchableOpacity
              ...
              disabled={(createStoryMutation as any).isLoading}
            >
              <Text style={styles.postButtonText}>
                {(createStoryMutation as any).isLoading ? "Posting..." : "Post Story"}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <DeleteModal
        {...({
          ...
          loading: (deleteStoryMutation as any).isLoading
        } as any)}
      />
    </View>
  );
